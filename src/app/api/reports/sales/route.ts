...
export async function GET(request: NextRequest) {
  try {
    ...
    const previousPeriodSales = await prisma.salesOrder.findMany({
      where: {
        orderDate: {
    ...
    const previousRevenue = previousPeriodSales.reduce((sum, order) => {
      return (
        sum +
        Number(
          order.salesOrderLines.reduce(
    ...
    // Sales by category
    const salesByCategory = await prisma.salesOrderLine.groupBy({
      by: ["itemId"],
      where: {
    ...
    const topCustomersData = await Promise.all(
      topCustomers.map(async (customer) => {
        ...
        return {
          ...
          amount: Number(customer._sum.totalAmount || 0),
    ...
    return NextResponse.json({
      ...
      salesByCategory: Object.entries(categorySummary).map(
        ([category, data]) => ({
          category,
          amount: data.amount,
          qty: data.qty,
